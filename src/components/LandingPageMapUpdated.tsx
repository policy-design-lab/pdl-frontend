...
const drawLegend = (programData, colorScale, [color1, color2, color3, color4, color5], mes) => {
    // Updated Test Legend
    const rn = React.useRef(null);
    const prepColor = [color1, color2, color3, color4, color5];
    let cut_points: number[] = [];
    const temp = colorScale.quantiles();
    ...
    if (cut_points.length > 0) {
        ...
        base.append("text")
            ...
            .attr("y", 15)
            .text(
                "Current setting with automated marks. Equally split to 5 groups and each interval have roughly same number of states."
            );
        base.append("text")
            .attr("class", "legendTitle")
            .attr("x", 10)
            .attr("y", 35)
            .text(`Scale is ${temp.toString()}. Detect that ${message} has zero points`);
        base.append("rect")
        ...
        base.append("text")
            ...
            .text((d, i) => {
                return i !== cut_points.length - 1
                    ...
                    : `${ShortFormat(Math.round(cut_points[i]))} to ${ShortFormat(Math.max(...programData))}`;
            });
    }
    return (
        <svg ref={rn} className="MapLegendSVG" height={250} width={1000}>
