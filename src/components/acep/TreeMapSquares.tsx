...
export default function TreeMapSquares({
    ...
    React.useEffect(() => {
        if (chartData) {
            ...
            const d = chartData.filter(
                (stateData) => stateData.acres !== 0 || stateData.payments !== 0 || stateData.contracts !== 0
            );
            let i = 0;
            let lineNumber = 0;
            let maxInLine = 0;
            while (i < d.length) {
                ...
                if (
                    rowTrack + margin <=
                    svgWidth - largestSquare * Math.max(stateData.acres, stateData.payments, stateData.contracts)
                ) {
                    const squareGroup = base.append("g").attr("class", "squareGroup");
                    const re_sorted = [stateData.acres, stateData.payments, stateData.contracts].sort((a, b) => b - a);
                    maxInLine = re_sorted[0];
                    const acresOriginalData = ShortFormat(originalData.find((s) => s.state === stateData.state).acres);
                    const paymentsOriginalData = ShortFormat(
                        originalData.find((s) => s.state === stateData.state).payments
                    );
                    const contractsOriginalData = ShortFormat(
                        originalData.find((s) => s.state === stateData.state).contracts
                    );
                    const collectedOriginalData = {
                        acres: acresOriginalData,
                        payments: paymentsOriginalData,
                        contracts: contractsOriginalData
                    };
                    ...
                    squareGroup
                        .on("mouseover", function (e) {
                            ...
                            tipGroup
                                .append("text")
                                .text(`Total Benefits:  $${paymentsOriginalData}`)
                                .attr("x", xPosition + baseSize)
                    ...
                    const stateName = squareGroup
                        .append("text")
                        .text(Object.values(stateCodes).filter((stateCode) => stateCode === stateData.state)[0])
                    ...
                    for (let temp = 1; temp <= 5; temp += 1) {
                        if (chartData[temp + 1]) {
                            ...
                            max =
                                max > [nextOne.acres, nextOne.payments, nextOne.contracts].sort((a, b) => b - a)[0]
                                    ? max
                                    : [nextOne.acres, nextOne.payments, nextOne.contracts].sort((a, b) => b - a)[0];
                        }
                    }
                    if (max <= 0.8) {
    ...
    return (
        <Styles>
            <svg ref={rn} id="AcepTreeMap" width={svgWidth} height={svgHeight} />
        </Styles>
    );
}
