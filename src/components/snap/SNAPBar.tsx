  ...
  const renderBar = (status) => {
    ...
    const purpleLabel = (purpleBar, state, status) => {
      if (status === 0 || status === 2) {
        ...
        const mousePos = d3.pointer(
          event,
          d3.select(rn.current).select(".y1.axis").node()
        );
        ...
        const rightLineData = [
          {
            x:
              ...
              mousePos[0] -
              minusMouse[0] +
              parseFloat(purpleBar.attr("width")),
            y: purpleBar.attr("y"),
          },
    ...
    const purpleText = (purpleBar, state, status) => {
      const rightTextContent = `Participation: ${ToDollarString(
        ...
        purpleBar.data()[0]["averageMonthlyParticipationInPercentageNationwide"]
      )}`;
      const rightText = d3
        .select(rn.current)
        .append("text")
    ...
    const blueLabel = (blueBar, state, status) => {
      ...
      if (status === 0 || status === 1) {
        ...
        const mousePos = d3.pointer(
          event,
          d3.select(rn.current).select(".y0.axis").node()
        );
    ...
    const blueText = (blueBar, state, status) => {
      const leftTextContent = `Costs: $${ToDollarString(
        ...
        blueBar.data()[0]["totalPaymentInPercentageNationwide"]
      )}`;
      const leftText = d3
        .select(rn.current)
        .append("text")
    ...
    x1.domain(["totalPaymentInDollars", "averageMonthlyParticipation"]).range([
      0,
      x0.bandwidth(),
    ]);
    y0.domain([
    ...
    const x_axis = d3
      ...
      .attr(
        "transform",
        `translate(${margin.left},${topSpace + graphHeight + margin.top})`
      )
      .call(xAxis);
    ...
    d3.select(rn.current)
      ...
      .attr(
        "transform",
        `translate(${graphWidth + margin.left}, ${topSpace + margin.top})`
      )
      .call(yAxisRight)
      .append("text")
      .attr("y", -margin.top)
    ...
    d3.select(rn.current)
      ...
      .attr(
        "transform",
        `translate(${margin.left + graphWidth}, ${topSpace + margin.top})`
      )
      .call(y1AxisGrid)
      .selectAll(".tick")
      .style("opacity", 0.1);
