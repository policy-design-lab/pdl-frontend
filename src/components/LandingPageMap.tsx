...
const [selectedPrograms, setSelectedPrograms] = useState(["All Programs"]);
    ...
    const calculateTotal = (state) => {
        if (selectedPrograms.includes("All Programs")) {
            return allPrograms.find(s => s.State === state)?.["18-22 All Programs Total"] || 0;
        }
    ...
    const colorScale = useMemo(() => {
        const allTotals = allStates.map(state => calculateTotal(state.id));
        const customScale = d3.scaleQuantile()
            .domain(allTotals)
    ...
    const handleProgramChange = (event) => {
        ...
        setSelectedPrograms(
            value.includes("All Programs") ? ["All Programs"] : value.filter(v => v !== "All Programs")
        );
    };
    return (
        <div data-tip="">
            <Box display="flex" justifyContent="center" alignItems="center" mb={2}>
                <FormControl sx={{ m: 1, width: 300 }}>
                    <InputLabel id="program-select-label">Programs</InputLabel>
                    <Select
            ...
            <Box id="TopMapContainer" display="flex" justifyContent="center" sx={{ mt: 4 }}>
                <DrawLegend
                    ...
                    title={
                        <Typography variant="h6">
                            Total Farm Bill Benefits Test from <strong>2018 - 2022</strong>
                        </Typography>
                    }
                    programData={allStates.map(state => calculateTotal(state.id))}
                    prepColor={["#FFF9D8", "#E1F2C4", "#9FD9BA", "#1B9577", "#005A45"]}
                    emptyState={[]}
                    initRatioLarge={0.75}
                    initRatioSmall={0.8}
                                                    ...
                                                    {selectedPrograms.map(program => (
                                                        <tr key={program}>
                                                            ...
                                                            <td className={classes.tooltip_regularcell_right}>
                                                                ${ShortFormat(
                                                                    summary
                                                                        .filter(item => item.State === cur.id && item.Title === program)
                                                                        .reduce((sum, item) => sum + item.Amount, 0),
                                                                    undefined,
                                                                    2
                                                                )}
