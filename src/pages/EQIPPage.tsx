...
export default function EQIPPage(): JSX.Element {
  ...
  const cur1 = chartData.statutes.find(
		(s) => s.statuteName === "(6)(A) Practices"
	);
	const cur2 = chartData.statutes.find(
	...
	const structuralCur = ACur.find(
		(s) => s.practiceCategoryName === "Structural"
	);
	const landManagementCur = ACur.find(
		(s) => s.practiceCategoryName === "Land management"
	);
	const vegetativeCur = ACur.find(
		(s) => s.practiceCategoryName === "Vegetative"
	);
	const forestManagementCur = ACur.find(
		(s) => s.practiceCategoryName === "Forest management"
	);
	const soilRemediationCur = ACur.find(
		(s) => s.practiceCategoryName === "Soil remediation"
	);
	const other6ACur = ACur.find(
		(s) => s.practiceCategoryName === "Other improvement"
	);
	const soilTestingCur = ACur.find(
		(s) => s.practiceCategoryName === "Soil testing"
	);

	const otherPlanningCur = BCur.find(
		(s) => s.practiceCategoryName === "Other planning"
	);
	const conservationPlanningAssessmentCur = BCur.find(
		(s) => s.practiceCategoryName === "Conservation planning assessment"
	);
	const comprehensiveNutrientMgtCur = BCur.find(
		(s) => s.practiceCategoryName === "Comprehensive Nutrient Mgt."
	);
	const resourceConservingCropRotationCur = BCur.find(
		(s) => s.practiceCategoryName === "Resource-conserving crop rotation"
	);
	const soilHealthCur = BCur.find(
		(s) => s.practiceCategoryName === "Soil health"
	);
	...
	conservationPlanningAssessmentTotal += Number(
		conservationPlanningAssessmentCur.totalPaymentInDollars
	);
	comprehensiveNutrientMgtTotal += Number(
		comprehensiveNutrientMgtCur.totalPaymentInDollars
	);
	resourceConservingCropRotationTotal += Number(
		resourceConservingCropRotationCur.totalPaymentInDollars
	);
	...
	const sixAChartData = [
		...
		{
			name: "Soil remediation; Soil testing",
			value: soilRemediationTotal + soilTestingTotal,
			color: "#CDDBD8",
		},
	];
	...
	const sixBChartData = [
		...
		{
			name: "Conservation planning assessment",
			value: conservationPlanningAssessmentTotal,
			color: "#4D847A",
		},
		{
			name: "Comprehensive Nutrient Mgt.",
			value: comprehensiveNutrientMgtTotal,
			color: "#749F97",
		},
		{
					...
					>
						<Box display="flex" justifyContent="center">
							<Typography variant="h5">
								<strong>
                  EQIP: State Performance by Category of Practices
								</strong>
							</Typography>
						</Box>
						<Typography sx={{ mt: 2 }}>
              EQIP provides cost-share assistance for improvements to eligible
              land. In the statute, Congress defined seven categories of
              conservation practices: (1) structural practices, such as for
              irrigation and livestock manure management or abatement; (2) land
              management practices, such as for fencing, drainage water
              management, grazing, prescribed burning, and wildlife habitat; (3)
              vegetative practices, such as planting filter strips, cover crops,
              grassed waterways, field borders, windbreaks, and shelterbelts;
              (4) forest management practices, which include planting trees and
              shrubs, improving forest stands, planting riparian forest buffers,
              and treating residues; (5) soil testing practices; (6) soil
              remediation practices, such as residue and tillage management
              (no-till, mulch-till, or strip-till), and amendments for treating
              agricultural wastes; and (7) other practices, including integrated
              pest management, dust control, and energy improvements.
						</Typography>
					</Box>
					<Box
						component="div"
